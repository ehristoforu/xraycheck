...
def _date_from_response_headers(headers: Mapping[str, str]) -> str:
    """Из заголовков ответа (Last-Modified или Date) возвращает дату в формате YYYY-MM-DD или пустую строку."""
    lm = headers.get("Last-Modified") or headers.get("Date") or ""
    if not lm:
        return ""
    try:
        ...
        return lm[:16] if len(lm) > 16 else lm
...
def fetch_list_with_meta(url: str) -> tuple[str, str]:
    """Загружает список по URL и возвращает (текст, дата_обновления YYYY-MM-DD из Last-Modified/Date)."""
    try:
        parsed = urlparse(url)
        if not parsed.scheme or not parsed.netloc:
    ...
    r = requests.get(url, timeout=15)
    r.raise_for_status()
    date_str = _date_from_response_headers(r.headers)
    return decode_subscription_content(r.text), date_str
...
def load_merged_keys(links_file: str) -> tuple[str, list[tuple[str, str]]]:
    ...
    В лог выводится таблица по источникам: №, дата обновления, количество ключей (без URL).
    """
    ...
    result: list[tuple[str, str]] = []
    total_urls = len(urls)
    source_stats: list[tuple[int, str, int]] = []  # (номер, дата, ключей)

    def _short_error(msg: str) -> str:
        """Убирает URL из сообщения об ошибке и обрезает длину."""
    ...
    with Progress(
        ...
        for idx, url in enumerate(urls, 1):
            try:
                text, date_str = fetch_list_with_meta(url)
                ...
                for link, full in parsed:
                    if link not in seen_links:
                        ...
                        new_count += 1
                source_stats.append((idx, date_str or "-", len(parsed)))
                ...
                error_msg = _short_error(str(e))
                source_stats.append((idx, "-", 0))
    ...
    # Таблица по источникам: номер, дата обновления, количество ключей (без URL)
    console.print("")
    console.print("[bold]Статистика по источникам (без URL):[/bold]")
    console.print("| № | Дата обновления | Ключей |")
    console.print("|---|-----------------|--------|")
    for num, date, count in source_stats:
        console.print(f"| {num} | {date} | {count} |")
    console.print("")
